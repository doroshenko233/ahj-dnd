export default class Taskmanager {
  ...
  init() {
    ...
    [...this.columns].forEach((element) => {
      element.addEventListener("mouseover", this.closeItemMouseOver);
    });
    [...this.columns].forEach((element) => {
      element.addEventListener("mouseout", this.closeItemMouseOut);
    });
    [...this.columns].forEach((element) => {
  ...
  addItem(event) {
    ...
    addForm.innerHTML = `
    <textarea class="taskmanager__add_textarea" placeholder="Введите что-нибудь..."></textarea>
  ...
  addItemSubmit(event) {
    ...
    if (textarea.value) {
      const currentColumn =
        event.target.parentElement.querySelector(".taskmanager__list");
  ...
  addItemClose(event) {
    const currentColumn = event.target
      .closest(".taskmanager__column")
      .querySelector(".taskmanager__list");
    const columnIndex = [...this.columns].findIndex(
      (column) => column === currentColumn,
    );
    event.target
      .closest(".taskmanager__add_form")
      .replaceWith(this.add[columnIndex]);
  }
  ...
  closeItemMouseOut(event) {
    ...
    const closeParentElement = event.target.parentNode.querySelector(
      ".taskmanager__close",
    );
    if (closeElement) {
      closeElement.remove();
  ...
  closeItem(event) {
    ...
    const itemIndex = this.tasks[columnIndex].findIndex(
      (item) => item === itemText,
    );
  ...
  dragStart(event) {
    if (
      !event.target.classList.contains("taskmanager__item") ||
      event.which !== 1
    ) {
      return;
    }
    ...
    [...this.columns].forEach((element) => {
      element.addEventListener("mousemove", this.dragTakePlace);
    });
    [...this.columns].forEach((element) => {
      element.addEventListener("mouseleave", this.dragPlaceRemove);
    });
  ...
  dragClean() {
    ...
    [...this.columns].forEach((element) => {
      element.removeEventListener("mousemove", this.dragTakePlace);
    });
    [...this.columns].forEach((element) => {
      element.removeEventListener("mouseleave", this.dragPlaceRemove);
    });
  ...
  tasksCollect() {
    ...
    for (let i = 0; i < this.columns.length; i += 1) {
      const tasksElement =
        this.columns[i].querySelectorAll(".taskmanager__item");
